...
class ConDebaterAgent(ChildDebaterAgent, GroqMixin):
    """
    Agent specialized in arguing against the existence of extraterrestrial life.

    This agent utilizes the Groq API to generate skeptical, evidence-based
    ...
    def __init__(
        ...
        """Initialize the con agent with persona and Groq model."""
        ...
        role_instruction = "You are the con debater. You must argue against the topic."
        system_prompt = (
            f"{role_instruction}\n{base_persona}\n\nDEBATE PROTOCOLS:\n{rules}\n\n{fmt}"
        )
